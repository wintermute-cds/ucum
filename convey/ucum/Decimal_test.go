...
func TestNewFromStringDeepEquals(t *testing.T) {
	...
	for _, cmp := range tests {
		d1, err1 := ucum.NewFromString(cmp.str1)
		d2, err2 := ucum.NewFromString(cmp.str2)
...
func TestBadJSON(t *testing.T) {
	for _, testCase := range []string{
		...
		var doc struct {
			Amount ucum.Decimal `json:"amount"`
		}
...
func TestDecimal_Floor(t *testing.T) {
	...
	for _, test := range tests {
		d, _ := ucum.NewFromString(test.input)
		expected, _ := ucum.NewFromString(test.expected)
		got := d.Floor()
...
func TestDecimal_RoundAndStringFixed(t *testing.T) {
	...
	for _, test := range tests {
		d, err := ucum.NewFromString(test.input)
		if err != nil {
			panic(err)
		}

		// test Round
		expected, err := ucum.NewFromString(test.expected)
		if err != nil {
			panic(err)
		}
...
func TestDecimal_Add(t *testing.T) {
	...
	for inp, res := range inputs {
		a, err := ucum.NewFromString(inp.a)
		if err != nil {
			t.FailNow()
		}
		b, err := ucum.NewFromString(inp.b)
		if err != nil {
			t.FailNow()
		}
		c := a.Add(b)
...
func TestDecimal_Div(t *testing.T) {
	...
	for inp, expectedStr := range inputs {
		num, err := ucum.NewFromString(inp.a)
		if err != nil {
			t.FailNow()
		}
		denom, err := ucum.NewFromString(inp.b)
		if err != nil {
			t.FailNow()
		}
		got := num.Div(denom)
		expected, _ := ucum.NewFromString(expectedStr)
		if !got.Equal(expected) {
			t.Errorf("expected %v when dividing %v by %v, got %v",
				expected, num, denom, got)
		}
		got2 := num.DivRound(denom, int32(ucum.DivisionPrecision))
...
func TestDecimal_DivRound(t *testing.T) {
	...
	for _, s := range cases {
		d, _ := ucum.NewFromString(s.d)
		d2, _ := ucum.NewFromString(s.d2)
		result, _ := ucum.NewFromString(s.result)
		prec := s.prec
...
func TestIntPart(t *testing.T) {
	for _, testCase := range []struct {
		...
		d, err := ucum.NewFromString(testCase.Dec)
		if err != nil {
			t.Fatal(err)
		}
...
func TestDecimal_Min(t *testing.T) {
	...
	for _, test := range testCases {
		...
		got := ucum.Min(decimalInput[0], decimalInput[1:]...)
...
func TestDecimal_Max(t *testing.T) {
	...
	for _, test := range testCases {
		...
		got := ucum.Max(decimalInput[0], decimalInput[1:]...)
		if !got.Equal(expectedDecimal) {
			t.Errorf("Expected %v, got %v, input=%+v", expectedDecimal, got,
				decimalInput)
		}
...
func TestDecimal_Scan(t *testing.T) {
	...
	expected, err = ucum.NewFromString(valueStr)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNullDecimal_Value(t *testing.T) {
	// Make sure this does implement the database/sql's driver.Valuer interface
	var nullDecimal ucum.NullDecimal
...
func TestAvg(t *testing.T) {
	...
	avg := ucum.Avg(vals[0], vals[1:]...)
	if !avg.Equal(ucum.NewFromFloat(4.5)) {
		t.Errorf("Failed to calculate average, expected %s got %s", ucum.NewFromFloat(4.5).String(), avg.String())
	}
}

func TestNewFromFloat(t *testing.T) {
	for f, s := range testTable {
		d := ucum.NewFromFloat(f)
		if d.String() != s {
...
func TestNewFromFloatWithExponent(t *testing.T) {
	...
	for input, s := range tests {
		d := ucum.NewFromFloatWithExponent(input.float, input.exp)
		if d.String() != s {
			t.Errorf("expected %s, got %s (%s, %d)",
				s, d.String(),
				d.GetValue().String(), d.GetExp())
...
func TestNewFromBigIntWithExponent(t *testing.T) {
	...
	// add negatives
	for p, s := range tests {
		if p.val.Cmp(ucum.Zero.GetValue()) > 0 {
			tests[Inp{p.val.Neg(p.val), p.exp}] = "-" + s
		}
	}

	for input, s := range tests {
		d := ucum.NewFromBigInt(input.val, input.exp)
		if d.String() != s {
			t.Errorf("expected %s, got %s (%s, %d)",
				s, d.String(),
				d.GetValue().String(), d.GetExp())
...
func TestJSON(t *testing.T) {
	for _, s := range testTable {
		...
		ucum.MarshalJSONWithoutQuotes = true
		out, err = json.Marshal(&doc)
		if err != nil {
			t.Errorf("error marshaling %+v: %v", doc, err)
		...
		ucum.MarshalJSONWithoutQuotes = false
	}
}
...
func TestUnmarshalJSONNull(t *testing.T) {
	...
	if err != nil {
		t.Errorf("error unmarshaling %s: %v", docStr, err)
	} else if !doc.Amount.Equal(ucum.Zero) {
...
func TestDecimal_QuoRem(t *testing.T) {
	...
	for _, inp4 := range cases {
		d, _ := ucum.NewFromString(inp4.d)
		d2, _ := ucum.NewFromString(inp4.d2)
		prec := inp4.exp
		q, r := d.QuoRem(d2, prec)
		expectedQ, _ := ucum.NewFromString(inp4.q)
		expectedR, _ := ucum.NewFromString(inp4.r)
