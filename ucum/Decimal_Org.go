...
/**
    Precision aware Decimal_Org implementation. Any size number with any number of significant digits is supported.
  ...
  * This class is defined to work around the limitations of Java Big Decimal_Org
 *
 * @author Grahame
 *
 */
 ...
 /**
 Decimal_Org implementation for Golang for use in UCUM Service
  */
...
func NewDecimal_Org(value string)(*Decimal_Org, error){
	var err error
	d := &Decimal_Org{}
...
func NewDecimalInt_Org(value int)(*Decimal_Org, error){
	var err error
	d := &Decimal_Org{}
 ...
 */
func NewDecimalAndPrecision_Org(value string, precision int)(*Decimal_Org, error){
	var err error
	d := &Decimal_Org{}
	value = strings.ToLower(value)
	if strings.Contains(value, "e"){
...
func (d *Decimal_Org)setValueDecimal(value string)error{
	var err error
	d.Scientific = false
	dec := -1
...
func (d *Decimal_Org)allZeros(s string, start int)bool{
	for i,c := range s {
		if i >= start {
			if c != '0' {
				return false
...
func (d *Decimal_Org)countSignificants(value string)(int, error){
...
func (d *Decimal_Org)delete(value string, offset, length int)(string){
...
func (d *Decimal_Org)setValueScientific(value string)error {
	var err error
...
func (d *Decimal_Org)stringMultiply(c rune, i int)string{
	return PadLeft("", c, i)
}

func (d *Decimal_Org)insert(ins, value string, offset int)(string){
	if offset > len(value) {
...
func (d *Decimal_Org)String()string{
	return d.AsDecimal()
}

func (d *Decimal_Org)copy()*Decimal_Org {
	r := &Decimal_Org{}
...
func Zero_()(*Decimal_Org){
	d := &Decimal_Org{}
	d.setValueDecimal(strconv.Itoa(0))
	return d
}

func One()(*Decimal_Org){
	d := &Decimal_Org{}
	d.setValueDecimal(strconv.Itoa(1))
	return d
}

func (d *Decimal_Org)IsZero()bool{
	return d.allZeros(d.Digits,0)
}

func (d *Decimal_Org)IsOne()bool{
	one := One()
	return d.ComparesTo(one) == 0
}

func (d *Decimal_Org)Equals(other *Decimal_Org)bool{
	return d.ComparesTo(other) == 0
}

func (d *Decimal_Org)ComparesTo(other *Decimal_Org)int{
	if other == nil {
		return 0
	}
...
func (d *Decimal_Org) IsWholeNumber()(bool){
	s := d.AsDecimal()
	b := !strings.Contains(s, ".")
	return b
}

func (d *Decimal_Org)AsDecimal()(string){
	result := d.Digits
...
func (d *Decimal_Org)AsInteger()(int, error){
	b := d.IsWholeNumber()
	if !b {
		return 0, fmt.Errorf("Unable to represent "+d.String()+" as an integer")
	}
	dec, err := NewDecimalInt_Org(MIN_INT)
	if err!=nil {
		return 0, err
	}
	...
	dec, err = NewDecimalInt_Org(MAX_INT)
	if err!=nil {
		return 0, err
	}
	if d.ComparesTo(dec)>0 {
...
func (d *Decimal_Org)AsScientific()(string){
	result := d.Digits
...
func (d *Decimal_Org)Trunc()*Decimal_Org {
	if d.Decimal < 0 {
		return Zero_()
	}
...
func (d *Decimal_Org)Add(other *Decimal_Org)*Decimal_Org {
	if other == nil {
		return nil
	}
	result := d
...
func (d *Decimal_Org)Subtract(other *Decimal_Org)*Decimal_Org {
	if other == nil {
		return nil
	}
	result := d
...
func (d *Decimal_Org)makeEqualLength(other *Decimal_Org)(s1, s2 string, max int){
...
func (d *Decimal_Org)doAdd(other *Decimal_Org)*Decimal_Org {
...
func (d *Decimal_Org)doSubtract(other *Decimal_Org)*Decimal_Org {
	s1, s2, max := d.makeEqualLength(other)
	s3 := ""
...
func (d *Decimal_Org)stringAddition(s1, s2 string)string{
	if len(s1)!=len(s2){
...
func (d *Decimal_Org)stringSubtraction(s1, s2 string)string{
	if len(s1)!=len(s2){
		panic("string length assertion failed")
	}
...
func (d *Decimal_Org)replaceChar(s string, offset int, c rune)string{
	if offset == 0 {
		s = string(c) + s[1:]
	}else{
...
func (d *Decimal_Org)Multiply(other *Decimal_Org)*Decimal_Org {
	if other == nil {
		return nil
	}
...
func (d *Decimal_Org)Divide(other *Decimal_Org)*Decimal_Org {
	if other == nil {
		return nil
	}
	if d.IsZero() {
...
func (d *Decimal_Org)trimLeadingZeros(s string)string{
	if s == ""{
		return ""
	}
	i := 0
...
func (d *Decimal_Org)DivInt(other *Decimal_Org)*Decimal_Org {
	if other == nil {
		return nil
	}
	t := d.Divide(other)
...
func (d *Decimal_Org)Modulo(other *Decimal_Org)*Decimal_Org {
	if other == nil {
		return nil
	}
	t := d.DivInt(other)
...
func (d *Decimal_Org)Equal(value, maxDifference *Decimal_Org)bool{
	diff := d.Subtract(value).Absolute()
	return diff.ComparesTo(maxDifference) <= 0
}

func (d *Decimal_Org) Absolute()*Decimal_Org {
	di := d.copy()
	di.Negative = false
	return di
}
